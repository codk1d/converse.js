    ...
    initialize () {
        ...
        _.extend(_converse.api, {
            ...
            'roomviews': {
                 ...
                 * @returns {Promise} Promise which resolves with the Backbone.View representing the groupchat
                 *
                 * @example
                ...
                async get (jids) {
                    if (_.isArray(jids)) {
                        const views = await _converse.api.chatviews.get(jids);
                        return views.filter(v => v.model.get('type') === _converse.CHATROOMS_TYPE)
                    } else {
                        const view = await _converse.api.chatviews.get(jids);
                        if (view.model.get('type') === _converse.CHATROOMS_TYPE) {
                            return view;
                        } else {
                            return null;
